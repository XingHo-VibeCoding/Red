  ...
  useEffect(() => {
    ...
    const startGather = (fromScatter = true) => {
      ...
      if (fromScatter && !reducedMotion) {
        // 平滑飞散阶段：从当前位置逐渐飞出去，而不是瞬间传送
        particles.forEach(particle => {
          particle.startX = particle.x;
          particle.startY = particle.y;
          const angle = particle.seed * Math.PI * 2;
          const distance = spread * (0.35 + particle.depth * 0.75);
          particle.disperseX = particle.targetX + Math.cos(angle) * distance + (particle.depth - 0.5) * spread * 0.55;
          particle.disperseY = particle.targetY + Math.sin(angle) * distance + (particle.seed - 0.5) * spread * 0.55;
          particle.delay = particle.seed * stagger * 0.4;
        });
        phase = 'disperse';
      } else {
        particles.forEach(particle => {
    ...
    const render = now => {
      ...
      particles.forEach(particle => {
        ...
        if (phase === 'disperse') {
          const local = (now - gatherStart - particle.delay) / Math.max(1, disperseDuration);
          progress = clamp(local, 0, 1);
          const eased = easeOutCubic(progress);
          baseX = particle.startX + (particle.disperseX - particle.startX) * eased;
          baseY = particle.startY + (particle.disperseY - particle.startY) * eased;
